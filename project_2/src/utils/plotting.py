...
def heat_plot(data, filename, title="Heatmap", cmap="viridis", colorbar=True, annot=False):
    ...
    # Optional code block to format x-axis ticks
...
def loss_plot(data, filename, title="Loss Over Epochs", xlabel="Epochs", ylabel="Loss"):
    ...
    n_rows = (n_configs + n_cols - 1) // n_cols  # Calculate number of rows needed
    ...
    for idx, (lr, l2) in enumerate(unique_configs.values):
        ...
        # Optional code block to set subplot titles
    ...
    # Optional code block to set main title
...
def activation_plot(data, filename, title="Loss Over Epochs", xlabel="Epochs", ylabel="Loss"):
    """
    Plots and saves the loss over epochs for different activation functions in a single figure.

    Parameters:
        data (pd.DataFrame): DataFrame containing loss data with columns for "Epoch", "Loss", and "Activation_function".
        filename (str): Filename to save the plot.
        title (str): Title of the plot.
    ...
    # Optional code block to set title
...
if __name__ == "__main__":
    # Load and plot activation function data
