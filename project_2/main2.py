...
def classify():
    ...
    results_classification_df = pd.DataFrame(columns=["Learning Rate", "L2 Penalty", "Optimizer", "Model"])
    ...
    for lr in lrs:
        for l2 in l2s:
            for optim in optimizers:
                ...
                results_classification_df = pd.concat([results_classification_df,dfi], ignore_index=True)
...
if __name__ == '__main__':
    '''
    Below here is the classify function, it does FFNN and LR classification. 
    It should include the a pytorch comparing model.
    The grid search might not be as the project description wants.
    '''
    # classify()

    '''Below here should be regression. This can be turned into a function once complete.'''
    # Regression
    # Possible parameters to test:
    lrs = [0.0001,0.00001,0.000001]
    l2s = np.linspace(0.7,0.9,3)
